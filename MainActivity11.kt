...
NavHost(navController = navController, startDestination = "MyApp" )
{
    ...
    composable(route="Second/{p1}", arguments = listOf(navArgument("p1")
    {type= NavType.StringType
    defaultValue=""
        nullable=true
   },
        )){entry->SecondCompose(navController,entry.arguments?.getString("p1"))}
...
{ val p1="param1"
    ...
    Column(verticalArrangement = Arrangement.Center, horizontalAlignment = Alignment.CenterHorizontally) {
        ...
        Button(onClick = { navController.navigate("Second/$p1) }) {Text("next")
        }
    }
...
@Composable
fun SecondCompose(navController: NavController, p1: String?)
{
    Column(verticalArrangement = Arrangement.Center, horizontalAlignment = Alignment.CenterHorizontally) {
        Text(text = "Second Compose $p1 ")
